...
request = urllib.request.Request(
    "http://127.0.0.1:8787/v1/wechat/webview-probe",
...
try:
    ...
    raise SystemExit(f"WEBVIEW_PROBE_HTTP_ERROR={exc.code}: {body}") from exc
except urllib.error.URLError as exc:
    raise SystemExit(f"WEBVIEW_PROBE_UNREACHABLE={exc.reason}") from exc

if payload.get("sensitive_values_returned") is not False:
    raise SystemExit("WEBVIEW_PROBE_REJECTED=unexpected_sensitive_output_flag")
